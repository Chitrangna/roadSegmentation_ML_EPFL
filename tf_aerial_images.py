...
from submission_to_mask import binary_to_uint8 
...
tf.app.flags.DEFINE_string('train_dir', 'predictions/prediction1023/parameters',
...
def label_to_imgGrey(imgwidth, imgheight, w, h, labels):
...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    def get_prediction(img):
        ...
        # Get prediction for given input image
    def get_predictionGrey(img):
        data = numpy.asarray(img_crop(img, IMG_PATCH_SIZE, IMG_PATCH_SIZE))
        data_node = tf.constant(data)
        output = tf.nn.softmax(model(data_node))
        output_prediction = s.run(output)
        img_prediction = label_to_imgGrey(img.shape[0], img.shape[1], IMG_PATCH_SIZE, IMG_PATCH_SIZE, output_prediction)
        return img_prediction
    ...
    def get_prediction_with_groundtruth(filename, image_idx):
        ...
        # Get a concatenation of the prediction and groundtruth for given input file
    def get_prediction_with_groundtruthtest(filename, image_idx,isGrey):
        imageid = "test_" + str(image_idx)
        image_filename = filename + imageid +'/'+imageid + ".png"
        print(image_filename)
        img = mpimg.imread(image_filename)
        if(isGrey):
            img_prediction = get_predictionGrey(img)
        else:
            img_prediction = get_prediction(img)
        cimg = concatenate_images(img, img_prediction)
    ...
    def get_prediction_with_overlay(filename, image_idx):
        ...
        # Get prediction overlaid on the original image for given input file
    def get_prediction_with_overlaytest(filename, image_idx):
...
#         print("Running prediction on training set")
        prediction_training_dir = "predictions/prediction1023/training/"
        ...
        OUTPUT_PATH = 'submissions/results.csv'
    
        print("Running prediction on testing set")
        prediction_testing_dir = "predictions/prediction1023/testing/"
        if not os.path.isdir(prediction_testing_dir):
            os.mkdir(prediction_testing_dir)
        pimgs=[]
        for i in range(1, TESTING_SIZE + 1):
            pimg = get_prediction_with_groundtruthtest(test_data_filename, i,True)
            Image.fromarray(pimg).save(prediction_testing_dir + "greyprediction_" + str(i) + ".png")
            pimg = get_prediction_with_groundtruthtest(test_data_filename, i,False)
            Image.fromarray(pimg).save(prediction_testing_dir + "prediction_" + str(i) + ".png")
#             Image.fromarray(pimg).save("test0" + ".png")
            oimg = get_prediction_with_overlaytest(test_data_filename, i)
...
#             os.mkdir(prediction_testing_dir)
#         for i in range(1, TESTING_SIZE + 1):
#             pimg = get_prediction_with_groundtruthtest(test_data_filename, i)
#             Image.fromarray(pimg).save(prediction_testing_dir + "prediction_" + str(i) + ".png")
#             oimg = get_prediction_with_overlay(train_data_filename, i)
